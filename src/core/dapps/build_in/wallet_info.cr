...
module ::Axentro::Core::DApps::BuildIn
  ...
  class WalletInfo < DApp
    ...
    def wallet_info_impl(address)
      ...
      incoming_completed_transactions = incoming(address, "Completed", all_completed_transactions.select(&.recipients.map(&.address).includes?(address)))
      outgoing_completed_transactions = outgoing(address, "Completed", all_completed_transactions.select(&.senders.map(&.address).includes?(address)))

      incoming_pending_transactions = incoming(address, "Pending", (blockchain.pending_slow_transactions + blockchain.pending_fast_transactions).select(&.recipients.map(&.address).includes?(address)))
      outgoing_pending_transactions = outgoing(address, "Pending", (blockchain.pending_slow_transactions + blockchain.pending_fast_transactions).select(&.senders.map(&.address).includes?(address)))
