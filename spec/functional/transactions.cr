...
class Transactions < SpinachTestCase
  ...
  def send_custom_token(args)
    ...
    with_factory(developer_fund) do |block_factory, transaction_factory|
      ...
      if block_kind == "fast"
        block_factory.add_slow_block.add_slow_block.add_slow_block.add_slow_block.add_fast_block([kings_create_transaction]).add_fast_block([kings_send_transaction])
      else
